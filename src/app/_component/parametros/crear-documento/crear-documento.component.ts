...
export class CrearDocumentoComponent implements AfterViewInit {
  ...
  guardarFormaPago () {
    ...
    this.prestamosService.guardarDocumento (this.model).subscribe(
  ...
  submit() {
    ...
    if (this.form.valid) {
      ...
      if (this.html == '') {
        Swal.fire('Error', 'El contenido de la plantilla no puede estar vacio', 'error');
        return false;
      }
  ...
  getDatosFormasPago() {
    ...
    this.prestamosService.consultaPlantillasDocumentos ().subscribe(
      ...
      response => {
        ...
        let DATOS: documento[] = this.datosFormasPago;
        console.log(DATOS);
